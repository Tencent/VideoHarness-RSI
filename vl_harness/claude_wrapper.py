...
# Which Claude Code CLI binary to invoke. Default: official `claude` on PATH.
# Override with CLAUDE_CLI_BIN=/path/to/claude (or any compatible wrapper).
...
CLAUDE_CLI_BIN = _resolve_cli_bin()
...
def build_command(
    ...
    # non-interactive proposer still needs automatic file edits, so use the
    # supported acceptEdits mode there; retain the existing sandbox behavior
...
def run(
    ...
    env = os.environ.copy()
    # If CLAUDE_CLI_BIN points at a Node-based launcher, keep its sibling
    # runtime on PATH when present.
    cli_path = Path(CLAUDE_CLI_BIN)
    if cli_path.is_absolute() and (cli_path.parent / "node").is_file():
        env["PATH"] = f"{cli_path.parent}{os.pathsep}{env.get('PATH', '')}"
