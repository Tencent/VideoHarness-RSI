...
class ProviderLLM:
    ...
    def _call_completion(
        ...
        # Bypass system proxies (e.g. Squid) that intercept localhost/lan
        # requests to self-hosted OpenAI-compatible servers.
        call_kwargs.setdefault("proxy", None)
        if self.api_base:
            call_kwargs["base_url"] = self.api_base
            # Prefer: explicit self.api_key -> OPENAI_API_KEY env -> "local"
...
class LLM:
    ...
    def __call__(self, prompt: str) -> str:
        ...
        # vLLM/gpt-oss already splits reasoning vs final content; only
        # openrouter raw responses need harmony parsing.
        if self.is_gpt_oss and not self._provider.api_base:
